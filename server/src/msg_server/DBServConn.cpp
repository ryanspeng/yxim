...
extern uint32_t total_users, fail_users, send_msg_num;
extern uint32_t pc_total_users, android_total_users, ios_total_users, web_total_users;
extern CAes *pAes;
...
void CDBServConn::HandlePdu(CImPdu* pPdu)
{
	switch (pPdu->GetCommandId()) {
        ...
        case CID_MSG_UNREAD_CNT_RESPONSE:
            ...
            break;
        case CID_MSG_GET_DAYS_RES:
            _HandleGetMsgDayListResponse(pPdu);
            break;
        case CID_MSG_GET_DURING_TIME_RES:
            _HandleGetTimeMsgListResponse(pPdu);
            break;
...
void CDBServConn::_HandleGetMsgDayListResponse(CImPdu *pPdu)
{
    IM::Message::IMGetMsgDayListRsp msg;
    CHECK_PB_PARSE_MSG(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()));

    uint32_t user_id = msg.user_id();
    uint32_t session_type = msg.session_type();
    uint32_t session_id = msg.session_id();
    uint32_t msg_cnt = msg.msg_days_size();
    CDbAttachData attach_data((uchar_t*)msg.attach_data().c_str(), msg.attach_data().length());
    uint32_t handle = attach_data.GetHandle();
    
    log("HandleGetMsgListResponse, userId=%u, session_type=%u, opposite_user_id=%u, cnt=%u.", user_id, session_type, session_id, msg_cnt);
...
void CDBServConn::_HandleGetTimeMsgListResponse(CImPdu *pPdu)
{
    IM::Message::IMGetTimeMsgListRsp msg;
    CHECK_PB_PARSE_MSG(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()));
    ...
    if (pMsgConn && pMsgConn->IsOpen()) {
        if (pMsgConn->GetCryptoFlag() == 1)
        {
            for (int i=0; i < msg_cnt; i++)
            {
                ...
                pMsgConn->EncryptMsgData(msg_data);
                //log("HandleGetTimeMsgListRepsone, %u->%u data: %s", user_id, session_id, msg_data.c_str()); //debug crypto
...
void CDBServConn::_HandleMsgData(CImPdu *pPdu)
{
    ...
    send_msg_num++;
    log("HandleMsgData, from_user_id=%u, to_user_id=%u, msg_id=%u, token=%s.", from_user_id, to_user_id, msg_id, token.c_str());
    ...
    if (pMsgConn)
    {
        ...
        msg2.set_token(token);
        msg2.set_create_time(int2string(nNow));
        CImPdu pdu;
        pdu.SetPBMsg(&msg2);
    ...
    if (pFromImUser) {
        pFromImUser->BroadcastClientMsgData(pPdu, msg_id, msg_data, pMsgConn, from_user_id);
    }

    if (pToImUser) {
        pToImUser->BroadcastClientMsgData(pPdu, msg_id, msg_data, NULL, from_user_id);
    }
...
void CDBServConn::_HandleGetDeviceTokenResponse(CImPdu *pPdu)
{
    ...
    if (msg2.has_crypto_flag())
        log("HandleGetDeviceTokenResponse, %u->%u, crypto_flag=%u", from_id, to_id, msg2.crypto_flag());
    ...
    log("HandleGetDeviceTokenResponse, msg data = [%s].", msg_data.c_str());
    build_ios_d_push_flash(msg_data, msg2.msg_type(), from_id);
    //{
    //    "msg_type": 1,
    //    "from_id": "1345232",
    //    "group_type": "12353",
    ...
    uint32_t user_id = 0;
    uint32_t role = 0;
    IM::Server::IMPushToUserReq msg3_d;
    IM::Server::IMPushToUserReq msg3_s;
    for (uint32_t i = 0; i < user_token_cnt; i++)
    {
        ...
        if (pUser)
        {
            ...
            if (role == 1)
            {
                msg3_d.set_flash(msg_data);
                msg3_d.set_data(json_obj.json());
                user_token_tmp = msg3_d.add_user_token_list();
            }
            else
            {
                msg3_s.set_flash(msg_data);
                msg3_s.set_data(json_obj.json());
                user_token_tmp = msg3_s.add_user_token_list();
            }
            ...
            user_token_tmp->set_user_role(role);
    ...
    if (msg3_d.user_token_list_size() > 0)
    {
        CImPdu pdu3;
        pdu3.SetPBMsg(&msg3_d);
        pdu3.SetServiceId(SID_OTHER);
        pdu3.SetCommandId(CID_OTHER_PUSH_TO_USER_REQ);
        
        log("HandleGetDeviceTokenResponse, send push to doctor, size: %u", msg3_d.user_token_list_size());
        CDPushServConn* PushConn = get_d_push_serv_conn();
        if (PushConn) {
            PushConn->SendPdu(&pdu3);
        }
    }
    if (msg3_s.user_token_list_size() > 0)
    {
        CImPdu pdu3;
        pdu3.SetPBMsg(&msg3_s);
        pdu3.SetServiceId(SID_OTHER);
        pdu3.SetCommandId(CID_OTHER_PUSH_TO_USER_REQ);
        
        log("HandleGetDeviceTokenResponse, send push to patient, size: %u", msg3_s.user_token_list_size());
        CSPushServConn* PushConn = get_s_push_serv_conn();
        if (PushConn) {
            PushConn->SendPdu(&pdu3);
        }
    }

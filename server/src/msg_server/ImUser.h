...
public:
    ...
    void BroadcastClientMsgData(CImPdu* pPdu, uint32_t msg_id, string msg_data, CMsgConn* pFromConn = NULL, uint32_t from_id = 0);
    void BroadcastData(void* buff, uint32_t len, CMsgConn* pFromConn = NULL);
        
    void HandleKickUser(CMsgConn* pConn, uint32_t reason);
    
    //bool KickOutSameClientType(uint32_t client_type, uint32_t reason, uint32_t from_id);

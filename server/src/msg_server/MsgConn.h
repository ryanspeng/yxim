...
public:
    ...
    string GetKey() { return m_client_key; }
    void SetCryptoFlag(uint32_t flag) { m_crypto_flag = flag; }
    uint32_t GetCryptoFlag() { return m_crypto_flag; }
    ...
    int EncryptMsgData(string& msg_data);
    int DecryptMsgData(string& msg_data);
private:
    ...
    void _HandleClientGetMsgDayListRequest(CImPdu *pPdu);
    void _HandleClientGetTimeMsgListRequest(CImPdu *pPdu);
    ...
    void _HandleServerStatRequest(CImPdu *pPdu);
    void _HandleClientListRequest(CImPdu *pPdu);
private:
    ...
    uint32_t        m_crypto_flag;
    string          m_client_key;         //客户端传输加解密使用的Key
    CAes           *m_aes;
};

...
//bool CImUser::KickOutSameClientType(uint32_t client_type, uint32_t reason, uint32_t from_id)
bool CImUser::KickOutSameClientType(uint32_t client_type, uint32_t reason, CMsgConn* pFromConn)
{
    if (client_type == IM::BaseDefine::CLIENT_TYPE_WEB)
        return true;
    ...
    for (map<uint32_t, CMsgConn*>::iterator it = m_conn_map.begin(); it != m_conn_map.end(); it++)
    {
        ...
        //if (pMsgConn != NULL && pMsgConn->GetUserId() == from_id) {
        if (pMsgConn != NULL && pMsgConn != pFromConn) {

...
uint32_t total_users, fail_users, send_msg_num;
uint32_t pc_total_users, android_total_users, ios_total_users, web_total_users;
...
CMsgConn::CMsgConn()
{
    ...
    m_client_key = "";
    m_aes = NULL;
    m_client_type = IM::BaseDefine::CLIENT_TYPE_UNKNOWN;
}
...
{
    log("Close client, handle=%d, user_id=%u, key=%s ", m_handle, GetUserId(), GetKey().c_str());
...
void CMsgConn::HandlePdu(CImPdu* pPdu)
{
	...
	switch (pPdu->GetCommandId()) {
        ...
        case CID_MSG_TIME_REQUEST:
            ...
            break;
        case CID_MSG_GET_DAYS_REQ:
            _HandleClientGetMsgDayListRequest(pPdu);
            break;
        case CID_MSG_GET_DURING_TIME_REQ:
            _HandleClientGetTimeMsgListRequest(pPdu);
            break;
        ...
        case CID_BUDDY_LIST_DEPARTMENT_REQUEST:
            ...
            break;
        case CID_OTHER_SERVER_STATISTIC_REQ:
            _HandleServerStatRequest(pPdu);
            break;
        case CID_OTHER_CLIENT_LIST_REQ:
            _HandleClientListRequest(pPdu);
            break;
        // for group process
...
void CMsgConn::_HandleLoginRequest(CImPdu* pPdu)
{
    ...
    if (msg.has_crypto_flag() && msg.crypto_flag() == 1)
    {
        SetCryptoFlag(1);
    }
    else
    ...
    log("HandleLoginReq, user_name=%s, status=%u, client_type=%u, client=%s, crypto_flag=%u",
        m_login_name.c_str(), online_status, m_client_type, m_client_version.c_str(), GetCryptoFlag());
    ...
    msg2.set_client_type(m_client_type);
...
void CMsgConn::_HandleKickPCClient(CImPdu *pPdu)
{
    ...
    if (pImUser)
    {
        //pImUser->KickOutSameClientType(CLIENT_TYPE_MAC, IM::BaseDefine::KICK_REASON_MOBILE_KICK, user_id);
...
void CMsgConn::_HandleClientMsgData(CImPdu* pPdu)
{
    ...
    if (m_msg_cnt_per_sec >= MAX_MSG_CNT_PER_SECOND) {
        log("!!!too much msg cnt in one second, uid=%u msg_count=%u", GetUserId(), m_msg_cnt_per_sec);
        return;
    }
    ...
    if (GetCryptoFlag() == 1)
    {
        //log("HandleClientMsgData, %u->%u data: %s", GetUserId(), to_session_id, msg_data.c_str());  //debug crypto
        int ret = DecryptMsgData(msg_data);
        if (ret == 0)
        {
            msg.set_msg_data(msg_data);
            msg.set_crypto_flag(0);
            log("HandleClientMsgData, decrypt %d->%d msg", GetUserId(), to_session_id);
        }
        else
        {
            log("HandleClientMsgData, decrypt msg(%u->%u) failed", GetUserId(), to_session_id);
            return;
        }
    }
    if (g_log_msg_toggle) {
        log("HandleClientMsgData, %d->%d, msg_type=%u, msg_id=%u, token=%s. ", GetUserId(), to_session_id, msg_type, msg_id, token.c_str());
    }
...
void CMsgConn::_HandleServerStatRequest(CImPdu *pPdu)
{
    uint32_t cur_conns = 0;
    uint32_t cur_users = 0;
    uint32_t pc_cur_users = 0;
    uint32_t android_cur_users = 0;
    uint32_t ios_cur_users = 0;
    uint32_t web_cur_users = 0;
    ...
    for (ConnMap_t::iterator it = g_msg_conn_map.begin(); it != g_msg_conn_map.end(); )
    {
        ...
        cur_conns += 1;
        pConn = (CMsgConn*)it_old->second;
        string strKey = pConn->GetKey();
        if (strKey.length() > 0)
        {
            cur_users += 1;
            switch(pConn->GetClientType())
            {
    ...
    log("HandleServerStatRequest, connections=%u users=%u pc_users=%u android_users=%u ios_users=%u web_users=%u", cur_conns, cur_users, pc_cur_users, android_cur_users, ios_cur_users, web_cur_users);

    IM::Server::IMServerStatisticRsp msg;
    msg.set_total_users(total_users);
    msg.set_fail_users(fail_users);
    msg.set_cur_users(cur_users);
    msg.set_pc_total_users(pc_total_users);
    msg.set_pc_cur_users(pc_cur_users);
    msg.set_android_total_users(android_total_users);
    msg.set_android_cur_users(android_cur_users);
    msg.set_ios_total_users(ios_total_users);
    msg.set_ios_cur_users(ios_cur_users);
    msg.set_web_total_users(web_total_users);
    msg.set_web_cur_users(web_cur_users);
    msg.set_send_msg_num(send_msg_num);
    CImPdu pdu;
    pdu.SetPBMsg(&msg);
    pdu.SetServiceId(SID_OTHER);
    pdu.SetCommandId(CID_OTHER_SERVER_STATISTIC_RSP);
    pdu.SetSeqNum(pPdu->GetSeqNum());
    SendPdu(&pdu);
...
void CMsgConn::_HandleClientListRequest(CImPdu *pPdu)
{
    IM::Server::IMClientListRsp msg;
    ConnMap_t::iterator it_old;
    CMsgConn* pConn = NULL;
    ...
    for (ConnMap_t::iterator it = g_msg_conn_map.begin(); it != g_msg_conn_map.end(); )
    {
        ...
        if (strKey.length() > 0)
        {
            IM::BaseDefine::ClientInfo *info = msg.add_client_list();
            info->set_user_id(pConn->m_user_id);
            info->set_login_time(pConn->m_login_time);
            info->set_client_type(pConn->m_client_type);
            info->set_client_version(pConn->m_client_version);
        }
    }
    ...
    pdu.SetPBMsg(&msg);
    pdu.SetServiceId(SID_OTHER);
    pdu.SetCommandId(CID_OTHER_CLIENT_LIST_RSP);
    pdu.SetSeqNum(pPdu->GetSeqNum());
    SendPdu(&pdu);
}

void CMsgConn::_HandleClientGetMsgDayListRequest(CImPdu *pPdu)
{
    IM::Message::IMGetMsgDayListReq msg;
    CHECK_PB_PARSE_MSG(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()));
    uint32_t session_id = msg.session_id();
    uint32_t session_type = msg.session_type();
    log("HandleClientGetMsgListRequest, req_id=%u, session_type=%u, session_id=%u. ",
...
void CMsgConn::_HandleClientGetTimeMsgListRequest(CImPdu *pPdu)
{
    IM::Message::IMGetTimeMsgListReq msg;
    CHECK_PB_PARSE_MSG(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()));
    uint32_t session_id = msg.session_id();
    uint32_t session_type = msg.session_type();
    uint32_t start_time = msg.start_time();
    uint32_t end_time = msg.end_time();
    uint32_t offset = msg.offset();
    uint32_t count = msg.count();
    log("HandleClientGetMsgListRequest, req_id=%u, session_type=%u, session_id=%u, start_time=%u, end_time=%u, offset=%u, count=%u. ",
        GetUserId(), session_type, session_id, start_time, end_time, offset, count);
    CDBServConn* pDBConn = get_db_serv_conn_for_login();
    if (pDBConn) {
        CDbAttachData attach(ATTACH_TYPE_HANDLE, m_handle, 0);
        msg.set_user_id(GetUserId());
...
void CMsgConn::_HandleClientDeviceToken(CImPdu *pPdu)
{
    if (GetUserId() == 0)
    {
        log("HandleClientDeviceToken, user_id=%u, not login.", GetUserId());
        return;
    }
    if (!CHECK_CLIENT_TYPE_MOBILE(GetClientType()))
    {
...
void CMsgConn::SetKey(string key)
{
    m_client_key = key;
    m_aes = new CAes(m_client_key);
}

int CMsgConn::EncryptMsgData(string &msg_data)
{
    char *msg_out = NULL;
    ...
    if (GetCryptoFlag() != 1)
    {
        log("EncryptMsgData, no set crypto");
        return 0;
    }
    ...
    int ret = m_aes->Encrypt(msg_data.c_str(), msg_data.length(), &msg_out, msg_out_len);
    if (ret == 0)
    {
    ...
    {
        log("EncryptMsgData, encrypt failed. error code: %d", ret);
    }
    return ret;
}

int CMsgConn::DecryptMsgData(string &msg_data)
{
    char *msg_out = NULL;
    uint32_t msg_out_len = 0;
    ...
    int ret = m_aes->Decrypt(msg_data.c_str(), msg_data.length(), &msg_out, msg_out_len);
    if (ret == 0)
    {
        msg_data = string(msg_out, msg_out_len);
        m_aes->Free(msg_out);
    ...
    {
        log("DecryptMsgData, decrypt failed. error code: %d", ret);
    }
    return ret;
}

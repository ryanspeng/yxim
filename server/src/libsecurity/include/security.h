...
extern "C" {
...
#ifdef __ANDROID__
    jbyteArray Java_com_mogujie_tt_Security_EncryptMsg(JNIEnv* env, jobject obj, jstring jkey, jstring jstr);
    jbyteArray Java_com_mogujie_tt_Security_DecryptMsg(JNIEnv* env, jobject obj, jstring jkey, jstring jstr);
     ...
     */
    DLL_MODIFIER int EncryptMsg(const char* key, const char* pInData, uint32_t nInLen, char** pOutData, uint32_t& nOutLen);
     ...
     *  @return 返回 0-成功; 其他-失败
     */
    DLL_MODIFIER int DecryptMsg(const char* key, const char* pInData, uint32_t nInLen, char** pOutData, uint32_t& nOutLen);

...
CResultSet::CResultSet(MYSQL_RES* res)
{
	...
	m_row_num = mysql_num_rows(m_res);
...
int CDBConn::Init()
{
	...
	mysql_options(m_mysql, MYSQL_SET_CHARSET_NAME, m_pDBPool->GetCharset());
...
CDBPool::CDBPool(const char* pool_name, const char* db_server_ip, uint16_t db_server_port,
		const char* username, const char* password, const char* db_name, const char* charset, int max_conn_cnt)
{
...
int CDBManager::Init()
{
	...
	char charset[64];
	char maxconncnt[64];
        char default_charset[64] = "utf8mb4";
	...
	for (uint32_t i = 0; i < instances_name.GetItemCnt(); i++) {
		...
		snprintf(charset, 64, "%s_charset", pool_name);
		...
		char* db_charset = config_file.GetConfigName(charset);
		char* str_maxconncnt = config_file.GetConfigName(maxconncnt);

		if (!db_charset) {
			db_charset = default_charset;
		}
		...
		CDBPool* pDBPool = new CDBPool(pool_name, db_host, db_port, db_username, db_password, db_dbname, db_charset, db_maxconncnt);
		if (pDBPool->Init()) {

...
bool checkUserName(const std::string &strName)
{
    for (int i=0; i < strName.size(); i++)
        if ((strName.at(i) < '0') || (strName.at(i) > '9'))
            return false;
    return true;
}
...
void doLogin(CImPdu* pPdu, uint32_t conn_uuid)
{
    ...
    if(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
    {
        ...
        uint32_t nClientType = msg.client_type();
        string strClientVersion = msg.client_version();
        
        msgResp.set_user_name(strDomain);
        msgResp.set_attach_data(msg.attach_data());
        
        if (!checkUserName(strDomain))
        {
            log("%s username invaild", strDomain.c_str());
            msgResp.set_result_code(3);
            msgResp.set_result_string("用户名不合法");
        ...
        if(g_loginStrategy.doLogin(strDomain, strPass, nClientType, strClientVersion, cUser))
        {
            ...
            pUser->set_user_domain(cUser.user_real_name());        //domain set real_name
            ...
            pUser->set_status(1);
            pUser->set_user_type(cUser.user_type());

            pUser->set_sign_info(cUser.sign_info());
           
            msgResp.set_result_code(0);
            msgResp.set_result_string("成功");
            log("%s login success.(uid=%d gender=%d nick:%s avatar:%s email:%s tel:%s name:%s type:%d)", strDomain.c_str(), cUser.user_id(), cUser.user_gender(),
                                                            cUser.user_nick_name().c_str(), cUser.avatar_url().c_str(), cUser.email().c_str(), cUser.user_tel().c_str(),
                                                            cUser.user_real_name().c_str(), cUser.user_type());
            DB_PROXY::setInternalDevicesToken(cUser.user_id(), "", (::IM::BaseDefine::ClientType)nClientType);

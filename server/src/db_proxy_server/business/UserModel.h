...
public:
    ...
    ~CUserModel();
    void getRelationId(uint32_t nUserId, uint32_t& nLastTime, list<uint32_t>& lsIds);
...
//    void getUserByNick(const list<string>& lsNicks, list<IM::BaseDefine::UserInfo>& lsUsers);
    ...
    bool getUserType(uint32_t user_id, uint32_t* user_type);
    bool updateUserSignInfo(uint32_t user_id, const string& sign_info);
    bool getUserSignInfo(uint32_t user_id, string* sign_info);

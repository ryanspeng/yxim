...
namespace DB_PROXY {
    ...
    void getMessageDays(CImPdu* pPdu, uint32_t conn_uuid)
    {
        ...
        if (msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
        {
            ...
            IM::Message::IMGetMsgDayListRsp msgResp;
            list<string> lsDays;

            CMessageModel::getInstance()->getMessageDays(nUserId, nPeerId, lsDays);

            msgResp.set_user_id(nUserId);
            msgResp.set_session_id(nPeerId);
            msgResp.set_session_type(nSessionType);
            for (auto it=lsDays.begin(); it!=lsDays.end(); ++it)
            {
                msgResp.add_msg_days(*it);
                log("userId=%u, peerId=%u, day=%s", nUserId, nPeerId, it->c_str());
            }

            log("userId=%u, peerId=%u, day_count=%u", nUserId, nPeerId, msgResp.msg_days_size());
            msgResp.set_attach_data(msg.attach_data());
            pPduResp->SetPBMsg(&msgResp);
            pPduResp->SetSeqNum(pPdu->GetSeqNum());
            pPduResp->SetServiceId(IM::BaseDefine::SID_MSG);
            pPduResp->SetCommandId(IM::BaseDefine::CID_MSG_GET_DAYS_RES);
    ...
    void getMessageDuringTime(CImPdu* pPdu, uint32_t conn_uuid)
    {
        ...
        if(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
        {
            uint32_t nUserId = msg.user_id();
            uint32_t nPeerId = msg.session_id();
            uint32_t nStartTime = msg.start_time();
            uint32_t nEndTime = msg.end_time();
            uint32_t nOffset = msg.offset();
            uint32_t nCount = msg.count();
            IM::BaseDefine::SessionType nSessionType = msg.session_type();

            CImPdu* pPduResp = new CImPdu;
            IM::Message::IMGetTimeMsgListRsp msgResp;
            list<IM::BaseDefine::MsgInfo> lsMsg;

            CMessageModel::getInstance()->getMessageDuringTime(nUserId, nPeerId, nStartTime, nEndTime, nOffset, nCount, lsMsg);
...
//                    log("userId=%u, peerId=%u, msgId=%u", nUserId, nPeerId, it->msg_id());
            ...
            log("userId=%u, peerId=%u, start=%u, end=%u, offset=%u, count=%u", nUserId, nPeerId, nStartTime, nEndTime, nOffset, msgResp.msg_list_size());
            msgResp.set_attach_data(msg.attach_data());
            pPduResp->SetPBMsg(&msgResp);
            pPduResp->SetSeqNum(pPdu->GetSeqNum());
            pPduResp->SetServiceId(IM::BaseDefine::SID_MSG);
            pPduResp->SetCommandId(IM::BaseDefine::CID_MSG_GET_DURING_TIME_RES);
            CProxyConn::AddResponsePdu(conn_uuid, pPduResp);
        }
        else
        {
        ...
        if(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
        {
            ...
            if (IM::BaseDefine::MsgType_IsValid(nMsgType))
            {
                if(nMsgLen != 0)
                {
                    ...
                    if (msg.has_forwarded() && msg.forwarded() == 1)
                    {
                        ...
                        CProxyConn::AddResponsePdu(conn_uuid, pPduResp);
                        log("Message forward to client [%u->%u(%u)]", nFromId, nToId, msg.msg_id());
                        return;
                    }
                        ...
                        if (nFromId != nToId) {
                            ...
                            if(nSessionId != INVALID_VALUE && nRelateId != INVALID_VALUE)
                            {
                                ...
                                if(nMsgId != INVALID_VALUE)
                                {
                                    ...
                                    string strUrl = "http://test.im.miaoshouapi.com:81/1/imweb/sendMsg";
                                    string strResp;
                                    string strMsgData = (string)msg.msg_data();
                                    string req_str = "from_id=" + int2string(nFromId) + "&to_id=" + int2string(nToId) + "&message=" + URLEncode(strMsgData);
                                    CURLcode nRet = httpClient.Post(strUrl, req_str, strResp);
                                    log("Send notify to interface, return=%u", nRet);
                                }
                                else
                                {

...
puts("db init success");
	...
	char* avatar_domain = config_file.GetConfigName("AvatarDomain");
	if (avatar_domain) {
		strAvatarDomain.assign(avatar_domain);
	} else {
		strAvatarDomain.assign("user.100xhs.com");
	}
